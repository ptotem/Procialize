class HomeController < ApplicationController
  ...
  def index
    ...
    @upcoming_meetings = Meeter.find_all_by_user_id_and_status(current_user.id, true).map { |r| r.meeting }.select{|m| m.start_time>Time.now and m.start_time<Time.now+900 }
    @upcoming_events=@conference.event_days.select{|ed| ed.event_date>Date.yesterday}.map{|ed| ed.events}.flatten.select{|e| Time.at(e.start.hour * 60 * 60 + e.start.min * 60 + e.start.sec)>@now-3600 and Time.at(e.start.hour * 60 * 60 + e.start.min * 60 + e.start.sec)<(@now+900) }
